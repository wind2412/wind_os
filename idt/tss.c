...
void tss_init()
{
	...
	//在IDT的第120项设置(任务门描述符)切换到用户	121项的切换到内核并不用修改。因为DPL是0x00，和其他中断没有区别.
	extern u32 _intrs[];
	set_intr_gate_desc(120, /*(u32)&_intrs[i]*/ _intrs[120], 0x08, IGD, 0x03, 1);		//与IDT项的不同之处只在于DPL：0x00->0x03  而IDT No.121并不用修改。
...
//	printf("%d\n", tss0.esp2);
	...
	frame->esp = &frame->esp;					//按照上边给出的链接，设置esp为&frame->esp。因为由报告书的图可知，iret要设置的esp实际上由于当时int调用并没有改变CPL，因此并没有push ss和esp进去。
...
{
	asm volatile ("int $121;");				//这里因为进行了CPL ring3->ring0的转换，因此CPU检测到之后，会自行压入ss3和esp3，并取出ss0和esp0(这个其实就是0，因为内核默认空栈。)
}
